...
places_2016 <- places_2016 %>% filter(StateAbbr == "WI") %>% rename(population_count = Population2010)
places_2017 <- places_2017 %>% filter(StateAbbr == "WI") 
places_2018 <- places_2018 %>% filter(StateAbbr == "WI") %>% mutate(TractFIPS = as.character(TractFIPS)) %>% rename(population_count = Population2010)
places_2019 <- places_2019 %>% filter(StateAbbr == "WI") %>% rename(population_count = Population2010) 
places_2020 <- places_2020 %>% filter(StateAbbr == "WI") %>% dplyr::select(-StateDesc) %>% mutate(Place_TractID = paste0(CountyFIPS,"-",TractFIPS)) %>% rename(population_count = TotalPopulation, PAPTEST_Crude95CI = CERVICAL_Crude95CI, PAPTEST_CrudePrev = CERVICAL_CrudePrev, PlaceFIPS = CountyFIPS, PlaceName = CountyName) %>% filter(TractFIPS %in% PLACES_FIPS)
...
write_csv(disease_all, paste0(wd,"Compiled PLACES data2.csv"))
```

```{r pre_load, include=FALSE}
...
# Use custom function #2 to impute median for missing values
...
# Use custom function #4 to apply transformation recommended by Box Cox method to predictor variables
...
# Apply custom function #3 to create ggpair plots (NOTE: this was done before and after transformation)
...
# Apply custom function #5 to perform PCA on key domains
...
# Examine PC loadings (custom function #6)
...
PCAloading_plot(sdoh_pca[[1]]$rotation, 7, .25)  # Housing quality
PCAloading_plot(sdoh_pca[[1]]$rotation, 8, .25)  # Income inequality?
PCAloading_plot(sdoh_pca[[1]]$rotation, 9, .25)  # Housing affordability?

PCAloading_plot(health_pca[[1]]$rotation, 1, .25)  # Health risk behaviors
...
Loading values were examined to provide an interpretation for each principal component. Meaningful interpretations were found for most principal components. Social determinant of health PC1 (Figure 5) loaded positively onto percent of households receiving food stamps, percent of households with less than \$25,000 in income, and percent of households living in poverty while loading negatively onto income per capita and median home value. This principal component is interpreted as a measure of poverty. Health PC3 (Figure 6) had strong positive loadings for distance from an emergency department, distance from a medical clinic, and percent uninsured. It is negatively associated with receiving a mammogram in the past year. This principal component is interpreted as a measure of access to health care.

### Figure 5. Principal component loadings for SDOH PC1

```{r graph5, echo=FALSE, fig.height=3.6, fig.align = "center"}
PCAloading_plot(sdoh_pca[[1]]$rotation, 1, .25) 
```

### Figure 6. Principal component loadings for Health PC3

```{r graph6, echo=FALSE, fig.height=3.5, fig.align = "center"}
PCAloading_plot(health_pca[[1]]$rotation, 3, .25)
```
...
for(jj in 1:kfolds) {
  ...
  # Summary table of best model for each fold
...
# Model assessment (RMSE, R squared, mean absolute error)
...
m1.assess.MAE = mae_calc(ycompare, m1predCV)  # custom function #7
...
# Plot predicted values versus actual values for double CV best model
...
# Plot pathway of best fit across diff lambdas
plot(model_final, xvar="lambda", label = TRUE)  
```

Finally, the best model (LASSO regression) was fit to the full data using the optimized control parameter (lambda=`r model_lasso$bestTune$lambda`). The LASSO model identified three predictor variables of greatest importance when predicting stroke crude prevalence (Figure 9): crude prevalence of blood pressure medication use, crude prevalence of binge drinking, and percent of adults who visited a doctor for a check up in the last year. Use of blood pressure medication and binge drinking were negatively related to crude prevalence of stroke. Visiting a doctor for a check up was positively associated with rates of stroke at the neighborhood level.

### Figure 9. Variable importance for LASSO regression

```{r graph9, echo=FALSE, fig.height=4, fig.align = "center"}
# Plot of variable importance for best model
final_model_varImp %>% ggplot(aes(x=reorder(Predictor, Importance), y=Importance, fill=Direction, color=Direction)) + 
  ...
  labs(y="Importance", x="") +
  ylim(0, max(final_model_varImp$Importance)) +
  theme_minimal() +
  theme(plot.title = element_text(size = 10), 
...
The predictors of greatest importance are both surprising and unsurprising. Higher prevalence of blood pressure medication usage was associated, as expected, with lower crude prevalence of stroke. Treating cardiovascular disease, a significant risk factor for stroke, with blood pressure medication is a protective health behavior. On an individual level, binge drinking is a known risk factor for stroke. However, in this analysis, higher prevalence of binge drinking was associated with lower prevalence of stroke. This may be explained by lower rates of binge drinking in census tracts with a greater percentage of households below the median income level (see Figure 2). Poverty is strongly associated with many adverse health outcomes, including stroke. Finally, census tracts reporting more visits to a doctor for a checkup in the past year were associated with higher prevalence of stroke. It is worth exploring whether these census tracts tended to be older, more female, or more sick in general as women, older adults, and those with chronic disease tend to be at higher risk for stroke and generally visit the doctor more often.

Based on this analysis, my recommendation to EMS providers interested in reducing the prevalence of stroke through community paramedicine is to increase screenings for high blood pressure in census tracts with high prevalence of stroke. Identifying and treating cardiovascular disease early will lead to better health outcomes for patients and will reduce burden on the EMS and healthcare systems.

```{r map_maker, echo=FALSE, include=FALSE}
# Load shapefile for Wisconsin census tracts
WI_shp <- st_read(paste0(wd,"WI_State_Census_Tracts_2018/tl_2018_55_tract.shp"))

# Join shapefile to disease data
places_2020 <- read_csv(paste0(wd,"PLACES__Census_Tract_Data__GIS_Friendly_Format___2020_release.csv"))
WI_shp <- left_join(WI_shp, places_2020, by=c('GEOID'='TractFIPS'))

# Select disease and place
WI_shp <- WI_shp %>% filter(TotalPopulation > 0, CountyName == 'Milwaukee')
...
map <- leaflet(WI_shp)  %>% addTiles() %>% addPolygons(fillColor = ~pal(disease), fillOpacity = 0.6, weight=1, color="white")

mapshot(map, file = paste0(wd,"MKEstrokes.png"))
```

### Crude prevalence of stroke in Milwaukee County in 2020
